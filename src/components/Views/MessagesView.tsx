...
type Status = 'available' | 'away' | 'dnd' | 'offline';
...
export function MessagesView() {
  ...
  const [userStatus, setUserStatus] = useState<Status>('available');
  ...
  const renderActiveChatTitle = (): ReactNode => {
  ...
  const renderAvatar = (chat: { type: 'stream' | 'dm', id: string }): ReactNode => {
     if (chat.type === 'dm') {
          ...
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56 glass-card text-white">
                  ...
                  <DropdownMenuSeparator />
                  {(Object.keys(statusOptions) as Status[]).map((key) => {
                      const {label, color} = statusOptions[key];
                      return (
          ...
          <CardContent className="flex-1 p-6 overflow-y-auto space-y-6">
             <AnimatePresence>
                {messages.length > 0 ? messages.map(msg => (
                  <motion.div key={msg.id} initial={{ opacity: 0, y:10}} animate={{ opacity: 1, y: 0}} className="flex items-start gap-3 group relative">
