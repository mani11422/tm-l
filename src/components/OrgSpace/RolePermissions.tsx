...
type Permission = 'view' | 'create' | 'update' | 'delete' | 'assign' | 'update_status' | 'manage_users' | 'manage_roles';
type Feature = 'tasks' | 'projects' | 'teams' | 'org';

const permissionsConfig: Record<UserRole, Record<Feature, Permission[]>> = {
  'Team Member': {
...
const allPermissions: Record<Feature, Permission[]> = {
    tasks: ['create', 'view', 'update_status', 'assign', 'delete'],
    projects: ['create', 'view', 'update', 'delete'],
    teams: ['create', 'view', 'update', 'delete'],
    org: ['manage_users', 'manage_roles'],
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-300">
            ...
            <tbody>
              {(Object.keys(allPermissions) as Feature[]).map((feature) => (
                <React.Fragment key={feature}>
                  {allPermissions[feature].map((perm, permIndex) => (
                    <tr key={`${feature}-${perm}`} className="border-b border-slate-800">
                      {permIndex === 0 && (
                        <td rowSpan={allPermissions[feature].length} className="px-6 py-4 font-medium text-white capitalize align-top border-r border-slate-800">
                          {feature}
                        </td>
                      )}
